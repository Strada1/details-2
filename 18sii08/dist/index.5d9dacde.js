...
if ((0, _storageJs.storage).getFavoriteCities()) {
    (0, _renderJs.renderList)();
    const currentCity = (0, _storageJs.storage).getCurrentCity();
    searchCity(currentCity);
}
async function searchCity(city = "") {
    let cityName;
    if (city === "") cityName = (0, _elementJs.ELEMENT).INPUT.value;
    else cityName = city;
    ...
    fetch(forecastUrl).then((response)=>response.json()).then((response)=>(0, _renderJs.addForecast)(response)).catch((err)=>alert(`Ошибка ${err.message}`));
}
(0, _elementJs.ELEMENT).SEARCH_FORM.addEventListener("submit", (event)=>{
    event.preventDefault();
    searchCity();
});
(0, _elementJs.ELEMENT).LIKE_IMG.addEventListener("click", (0, _renderJs.addCity));
(0, _elementJs.ELEMENT).CITYS.addEventListener("click", (event)=>{
    let city = event.target.textContent;
    searchCity(city);
});

},{"./render.js":"9k0mC","./element.js":"kPI0O","./storage.js":"j1l1C"}],"9k0mC":[function(require,module,exports) {
...
parcelHelpers.export(exports, "addForecast", ()=>addForecast);
var _elementJs = require("./element.js");
var _storageJs = require("./storage.js");
...
function render(response) {
    ...
    (0, _storageJs.storage).saveCurrentCity(response.name);
}
function renderNow(response) {
    let city = response.name;
    document.querySelector(".city-name").textContent = city;
    let temp = Math.round(response.main.temp - 273.15);
    (0, _elementJs.ELEMENT).DEGREE.textContent = temp;
    let img = response.weather[0].icon;
    console.log("img");
    (0, _elementJs.ELEMENT).INFO_IMG.src = `http://openweathermap.org/img/wn/${img}@2x.png`;
}
function renderDetails(response) {
    let city = response.name;
    document.querySelector(".details-title").textContent = city;
    let temp = Math.round(response.main.temp - 273.15);
    (0, _elementJs.ELEMENT).TEMP.textContent = temp;
    let feels = Math.round(response.main.feels_like - 273.15);
    (0, _elementJs.ELEMENT).FEELS.textContent = feels;
    let weather = response.weather[0].main;
    (0, _elementJs.ELEMENT).WEATHER.textContent = weather;
    let sunrise = new Date(response.sys.sunrise * 1000);
    (0, _elementJs.ELEMENT).SUNRISE.textContent = `${sunrise.getHours()}:${sunrise.getMinutes()}`;
    let sunset = new Date(response.sys.sunset * 1000);
    (0, _elementJs.ELEMENT).SUNSET.textContent = `${sunset.getHours()}:${sunset.getMinutes()}`;
}
function addCity() {
    let list = new Set((0, _storageJs.storage).getFavoriteCities());
    const city = (0, _elementJs.ELEMENT).CITY_NAME.textContent;
    if (city === "undefined" || !city) return;
    if (!list) {
        list.add(city);
        (0, _storageJs.storage).saveFavoriteCities(list);
        renderList();
        return;
    }
    if (list.has(city)) list.delete(city);
    else list.add(city);
    (0, _storageJs.storage).saveFavoriteCities(list);
    renderList();
}
function renderList() {
    ...
    for (let city of citys)city.remove();
    const list = Array.from((0, _storageJs.storage).getFavoriteCities());
    list.forEach((item)=>{
        for (let citys of document.querySelectorAll(".citys"))citys.insertAdjacentHTML("afterbegin", `<li class="city">${item}</li>`);
    });
}
...
function renderForecast(city, list) {
    ...
    for (let list1 of lists)list1.remove();
    ...
    for (let times of list){
        ...
        date.textContent = (0, _dateFns.format)(new Date(DATE.YEARS, DATE.MONTH, DATE.DAY), "dd MMMM");
        const temperature = createNode("p", "page_three__info__temperature");
        temperature.textContent = `Temperature: ${Math.round(times.main.temp - 273.15)}° Feels like: ${Math.round(times.main.feels_like - 273.15)}°`;
...
},{"./element.js":"kPI0O","./storage.js":"j1l1C","date-fns":"9yHCA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kPI0O":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const storage = {
    ...
    saveCurrentCity (city) {
        const inFifteenMinutes = new Date(new Date().getTime() + 3600000);
        (0, _jsCookieDefault.default).set("currentCity", city, {
    ...
    getCurrentCity () {
        return (0, _jsCookieDefault.default).get("currentCity");
    }
};
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["gzp3I","1Z4Rq"], "1Z4Rq", "parcelRequire39d6")
